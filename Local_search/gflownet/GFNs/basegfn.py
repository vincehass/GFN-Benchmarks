...
class BaseTBGFlowNet():
  ...
  def batch_fwd_sample_ls(self, n, epsilon=0.0, uniform=False, k=4, it=3, deterministic=False):
    ...
    for _ in range(it):
      ...
      #we get k=4 different backward and forward unique traj (same parent and children) k_forward_complete_trajs, logp_xprime2x - logp_x2xprime 
      ...
      print("lp_updates", lp_update)
      if deterministic:
        # Deterministic Filtering
        updates = (lp_update > 0).float()
        print("Filtering updates (1 if R(tau prime) > R(tau))", updates)
      else:
  ...
  def backforth_sample(self, trajs, k=4):
    ...
    print("k_backward_complete_trajs", k_backward_complete_trajs)
    for step in range(k):
      inp = [t[0] for t in k_backward_complete_trajs]
      print("input k_backward_complete_trajs", inp)
      #print("length input k_backward_complete_trajs", len(inp))
      #from complete traj sample parents for k-backward traj
      samples = self.back_sample(inp)
      print("back samples", samples)
      print("samples==inp",np.sum(samples == inp))
      logp_bs = self.back_logps_unique(inp)
      #print("logp_bs", logp_bs)
      for i, sample in enumerate(samples):
        k_backward_complete_trajs[i].insert(0, sample)
        #for each backward node traj append all their children
        print("k_backward_complete_trajs insert", k_backward_complete_trajs)
      #convert states samples into log softmax states
    ...
    for step in range(k):
      ...
      for i, (sample, logp_b) in enumerate(zip(inp, logp_bs)):
        #print("SAMPLES and LOG_B",sample, logp_b)
        logp_xprime2x[i] = logp_xprime2x[i] + logp_b[sample] 
          
      for i, (sample, logp_f) in enumerate(zip(samples, logp_fs)):
        #print("SAMPLES and LOG_F",sample, logp_f)
        logp_x2xprime[i] = logp_x2xprime[i] + logp_f[sample]
        print("\nk_forward_complete_trajs",k_forward_complete_trajs, "logp_xprime2x - logp_x2xprime\n",logp_xprime2x - logp_x2xprime)
